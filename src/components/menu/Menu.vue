<template>
  <div class="ui vertical accordion menu">
    <div class="item" v-for="menu in menus">
      <a class="title header" :class="{'active': $index==currentIndex,'selected-menu': (menu.name==currentMenu)}">
        <i class="icon" :class="menu.icon"></i>
        {{menu.title}}
        <i class="dropdown icon"></i>
      </a>
      <div class="content menu" :class="{'active': $index==currentIndex}">
        <a class="item" v-for="subMenu in menu.subMenu" v-link="{path: subMenu.path, exact: true}">
          {{subMenu.title}} </a>
      </div>
    </div>
  </div>
